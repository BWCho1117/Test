...
class PowerDependenceApp(QMainWindow):
    ...
    def load_data(self):
        ...
        for f in spe_files:
            try:
                ...
                power_val = 0
                if 'uW' in basename:
                    power_str = basename.split('uW')[0]
                    power_val = float(power_str)
                elif 'nW' in basename:
                    power_str = basename.split('nW')[0]
                    power_val = (float(power_str) / 1000.0)
                else:
                    power_str = basename.split('_')[0]
                    power_val = float(power_str)
    ...
    def plot_fit_results(self):
        ...
        if is_intensity_plot:
            y_key = 'integrated_intensity'
            y_label = 'Integrated Intensity (a.u.)'
            title = 'Power vs. Integrated Intensity'
            y_scale = 'log'
        elif self.plot_type_position_rb.isChecked():
            y_key = 'position'
            y_label = 'Peak Position (eV)'
            title = 'Power vs. Peak Position'
            y_scale = 'linear'
        elif self.plot_type_energy_diff_rb.isChecked():  # 추가
            y_key = 'position'
            y_label = 'Energy Diff (eV)'  # 에너지 차이로 레이블 변경
            title = 'Power vs. Energy Diff'  # 제목 변경
            y_scale = 'linear'
        else: # FWHM
            y_key = 'fwhm'
            y_label = 'FWHM (eV)'
