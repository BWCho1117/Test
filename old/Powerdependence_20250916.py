...
class FittingDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 추가: 피팅 라인셰이프 선택
        top_controls_layout.addWidget(QLabel("Lineshape:"), 0, 3)
        self.model_combo = QComboBox()
        self.model_combo.addItems(["Gaussian", "Lorentzian"])
        top_controls_layout.addWidget(self.model_combo, 0, 4)
    ...
    # 추가: Lorentzian (wid = HWHM)
    @staticmethod
    def lorentzian(x, amp, cen, wid):
        return amp * (wid**2) / ((x - cen)**2 + wid**2)

    # 추가: 현재 선택된 라인셰이프 반환
    def _current_peak_fn(self):
        return self.gaussian if self.model_combo.currentText() == "Gaussian" else self.lorentzian

    # 추가: 공통 멀티-모델
    def multi_model(self, x, *params):
        offset = float(self.bg_offset_edit.text())
        result = np.full_like(x, offset, dtype=float)
        fn = self._current_peak_fn()
        num_peaks = len(params) // 3
        for i in range(num_peaks):
            amp, cen, wid = params[i*3], params[i*3+1], params[i*3+2]
            result += fn(x, amp, cen, wid)
        return result
    ...
    def perform_fit(self):
        ...
        try:
            # 변경: 선택된 모델로 피팅
            popt, pcov = curve_fit(self.multi_model, self.energy_axis, self.spectrum_data,
                                   p0=p0, bounds=bounds, maxfev=10000)

            total_fit_curve = self.multi_model(self.energy_axis, *popt)
            individual_peaks_curves = []
            calculated_results = []

            peak_fn = self._current_peak_fn()
            use_gauss = (self.model_combo.currentText() == "Gaussian")

            for i in range(self.num_peaks):
                amp, cen, wid = popt[i*3:(i+1)*3]
                # 개별 피크 곡선
                peak_curve = peak_fn(self.energy_axis, amp, cen, wid) + bg_offset
                individual_peaks_curves.append(peak_curve)

                # FWHM/적분 강도 계산 (모델별)
                if use_gauss:
                    fwhm = 2 * np.sqrt(2 * np.log(2)) * wid          # wid = sigma
                    integrated_intensity = amp * wid * np.sqrt(2*np.pi)
                else:
                    fwhm = 2 * wid                                    # wid = HWHM
                    integrated_intensity = amp * np.pi * wid
