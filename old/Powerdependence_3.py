...
class PowerDependenceApp(QMainWindow):
    def __init__(self):
        ...
        self.is_energy_axis = False # 에너지 축 사용 가능 여부 플래그
        self.all_fit_results = {} # {power: {params...}, ...}
        self.recovery_file = 'fit_recovery.json'
        self.current_plot_mode = '2d_map' # 현재 플롯 모드 추적 ('2d_map' 또는 'analysis')
    ...
    def load_data(self):
        ...
        temp_data_list = []
        for f in spe_files:
            power_val, exposure_time = parse_power_exposure(f)
            # 표시용 문자열 생성
            power_str = f"{power_val * 1e9:.4f} nW" if power_val is not None else "N/A"
            exposure_str = f"{exposure_time:.2f} s" if exposure_time is not None else "N/A"
            display_str = f"{os.path.basename(f)} | Power: {power_str} | Exposure: {exposure_str}"
            self.spectra_list_widget.addItem(display_str)

            try:
                spe = SPE3read(f)
                if spe.data is not None and spe.data.size > 0:
                    spectrum = spe.data[0, 0, :]
                    temp_data_list.append((power_val, spectrum, spe.wavelength, spe.width))
                else:
        ...
        for power_val, _, _, _ in temp_data_list:
            item = QListWidgetItem(f"{power_val:.4f} uW")
    ...
    def plot_2d_map(self):
        ...
        try:
            c_min_text = self.color_min_edit.text()
            c_max_text = self.color_max_edit.text()
            x_min_text = self.x_axis_min_edit.text()
            x_max_text = self.x_axis_max_edit.text()
            y_min_text = self.y_axis_min_edit.text()
            y_max_text = self.y_axis_max_edit.text()

            c_min = float(c_min_text) if c_min_text else None
            c_max = float(c_max_text) if c_max_text else None
            
            x_min = float(x_min_text) if x_min_text else (self.energy_axis.min() if self.energy_axis is not None else 0)
            x_max = float(x_max_text) if x_max_text else (self.energy_axis.max() if self.energy_axis is not None else 1)
            
            y_min = float(y_min_text) if y_min_text else (min(self.power_values) if self.power_values else 0)
            y_max = float(y_max_text) if y_max_text else (max(self.power_values) if self.power_values else 1)

        except ValueError:
            QMessageBox.warning(self, "Input Error", "Please enter valid numbers for ranges.")
            return

        norm = colors.LogNorm(vmin=c_min, vmax=c_max) if self.color_log_rb.isChecked() else colors.Normalize(vmin=c_min, vmax=c_max)
        ...
        im = ax.imshow(smoothed_data, aspect='auto', origin='lower', extent=extent, norm=norm, cmap='magma')
        ...
        self.current_plot_mode = '2d_map' # 플롯 모드 설정
        self.canvas.draw()
    ...
    def fit_power_law(self):
        ...
        # '--' 스타일의 라인은 제외하고 원본 데이터 포인트 핸들만 가져옴
    ...
    def update_list_with_fit_status(self):
        for i in range(self.spectra_list_widget.count()):
            ...
            if power in self.all_fit_results:
                item.setText(f"{power:.4f} uW (Fitted)")
                item.setFont(QFont('Arial', 9, QFont.Bold))
            else:
                item.setText(f"{power:.4f} uW")
...
def parse_power_exposure(fname):
    """
    파일명에서 power와 exposure time을 추출
    예: 1nW_1s.spe → (1e-9, 1.0)
        1uW_0.5s.spe → (1e-6, 0.5)
        1nW.spe → (1e-9, None)
    """
    import os
    ...
    for part in parts:
        if part.endswith('uW'):
            try:
                power = float(part[:-2]) * 1e-6
            except ValueError:
                pass
        elif part.endswith('nW'):
            try:
                power = float(part[:-2]) * 1e-9
            except ValueError:
                pass
        elif part.endswith('s'):
            try:
                exposure = float(part[:-1])
            except ValueError:
                pass
    return power, exposure
