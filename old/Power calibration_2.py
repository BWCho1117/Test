...
class PowerCalibrationApp(QMainWindow):
    ...
    def perform_fit_and_plot(self):
        ...
        if len(voltages_for_fit) > 1:
            voltages_np = np.array(voltages_for_fit)
            powers_np = np.array(valid_powers)
            self.slope, self.intercept = np.polyfit(voltages_np, powers_np, 1)
        ...
        if voltages_for_fit:
            ax.scatter(voltages_for_fit, valid_powers, label='Data Points', color='red', zorder=5)
        if len(voltages_for_fit) > 1:
            min_v, max_v = min(voltages_for_fit), max(voltages_for_fit)
            fit_voltages = np.linspace(min_v, max_v, 100)
            fit_powers = self.slope * fit_voltages + self.intercept
            ax.plot(fit_voltages, fit_powers, label=f'Linear Fit (m={self.slope:.2f})', color='blue', zorder=10)
