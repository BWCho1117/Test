...
def choose_axis_mode(root: tk.Tk) -> str:
    win = tk.Toplevel(root)
    win.title("Select x-axis mode")
    win.resizable(False, False)
    try:
        win.attributes("-topmost", True)
    except Exception:
        pass
    mode_var = tk.StringVar(value="energy")
    frm = ttk.Frame(win, padding=12)
    frm.pack(fill="both", expand=True)
    ttk.Label(frm, text="x-axis:").pack(anchor="w", pady=(0, 8))
    ttk.Radiobutton(frm, text="Energy (eV)", value="energy", variable=mode_var).pack(anchor="w")
    ttk.Radiobutton(frm, text="Wavelength (nm)", value="wavelength", variable=mode_var).pack(anchor="w")
    btns = ttk.Frame(frm)
    ...
    win.grab_set()
    win.wait_window()
    return mode_var.get().strip().lower()
...
def choose_view_mode(root: tk.Tk) -> str:
    ...
    win.grab_set()
    win.wait_window()
    return "stack" if mode_var.get().strip().lower() == "stack" else "map"
...
def plot_map(I: np.ndarray, x: np.ndarray, B: np.ndarray, *, x_label: str, x_key: str, B_key: str, data_key: str, title: str):
    plt.figure()
    plt.imshow(
        I,
        aspect="auto",
...
def plot_stack(I: np.ndarray, x: np.ndarray, B: np.ndarray, *, x_label: str, x_key: str, B_key: str, data_key: str, title: str):
    nB = int(I.shape[0])
    stride = max(1, nB // 35)
...
def main():
    ...
    axis_mode = choose_axis_mode(_root)  # "energy" or "wavelength"
    ...
    # x-axis (energy or wavelength) + sort ascending
    if axis_mode == "energy":
        x = wavelength_nm_to_energy(wl, unit="eV")
        x_label = "Energy (eV)"
        x_key = f"{wl_key}→E"
    else:
