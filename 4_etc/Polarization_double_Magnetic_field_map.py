...
def plot_map(I: np.ndarray, wl: np.ndarray, B: np.ndarray, *, wl_key: str, B_key: str, data_key: str, title: str):
    plt.figure()
    plt.imshow(
        I,
        aspect="auto",
        origin="lower",
        extent=[float(wl.min()), float(wl.max()), float(B.min()), float(B.max())],
    )
    plt.xlabel(f"Wavelength (nm) [{wl_key}]")
...
def plot_stack(I: np.ndarray, wl: np.ndarray, B: np.ndarray, *, wl_key: str, B_key: str, data_key: str, title: str):
    ...
    for k, i in enumerate(idx):
        plt.plot(wl, I[i, :] + k * offset, lw=1.0)
...
def main():
    ...
    # sort wavelength ascending
    wl_idx = np.argsort(wl)
    wl = wl[wl_idx]
    I = I[:, wl_idx]
    ...
    if view_mode == "stack":
        plot_stack(I, wl, B, wl_key=wl_key, B_key=B_key, data_key=data_key, title=title)
    else:
        plot_map(I, wl, B, wl_key=wl_key, B_key=B_key, data_key=data_key, title=title)
